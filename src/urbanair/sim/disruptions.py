...
from ..enums import WeatherSeverity
...
def evolve_disruptions(
    ...
    rng,
    triggered_scripted_events: list[str] | None = None,
) -> tuple[list[str], int]:
    events: list[str] = []
    triggered_scripted_events = triggered_scripted_events or []
    ...
    if task_config.dynamic_events.no_fly and not task_config.deterministic_demo and tick in {3, 6, 8}:
    ...
    if task_config.difficulty.value == "hard" and tick in {4, 8}:
    ...
    if task_config.deterministic_demo and "failed_drop" in triggered_scripted_events:
        events.append("Scripted failed-drop pressure applied.")
